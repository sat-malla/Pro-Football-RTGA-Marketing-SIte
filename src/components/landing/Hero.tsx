...
export function Hero() {
  return (
    <section className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      ...
      />
      <div aria-hidden="true" className="yard-lines pointer-events-none absolute inset-0 opacity-40" />
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div>
          ...
          <p className="mt-7 max-w-xl text-lg text-muted-foreground">
            Spin up a full NFL roster at random from real, current player data! Run it through the machine-learning powered simulation and AI analysis trained on years of data. 
            Projected player stats, projected wins, projected playoff odds, Super Bowl odds. What capability does your team have?
          </p>
          ...
          <dl className="mt-12 grid max-w-3xl grid-cols-3 gap-4">
            {heroStats.map((s) => (
              <div key={s.label} className="panel px-4 py-4">
                ...
                <dt className="mt-1 text-xs leading-tight text-muted-foreground uppercase tracking-wider">{s.label}</dt>
              </div>
            ))}
          </dl>
        </div>
